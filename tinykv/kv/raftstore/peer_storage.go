...
func (ps *PeerStorage) Append(entries []eraftpb.Entry, raftWB *engine_util.WriteBatch) error {
	...
	for _, entry := range entries {
		...
		raftWB.SetMeta(meta.RaftLogKey(regionId, entry.GetIndex()), &entry)
...
func (ps *PeerStorage) SaveReadyState(ready *raft.Ready) (*ApplySnapResult, error) {
	...
	if len(ready.Entries) != 0 {
		...
		// add entries into `raftWB.entries` by `(*WriteBatch).SetMeta`
	...
	raftWB.MustWriteToDB(ps.Engines.Raft) // write all raft logs into disk through `raftWB` by badger
	return applyRes, nil
}
